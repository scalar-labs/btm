...
Sometimes it's not desirable to initialize the transaction manager automatically. For instance, you cannot use the `Configuration` object for configuration when automatic startup is enabled (default). You can disable this behavior with specifying the system property `bitronix.tm.osgi.disableActivator`. If set, the bundle activator will do nothing and you are responsible to initialize the transaction manager, and, to export it to the OSGi service registry.
