...
public class JdbcPreparedStatementHandle implements PreparedStatement {
    ...
    // The 'parent' connection.  Used to return the connection to the pool upon close().
    private JdbcPooledConnection parentConnection;

    // Brett Wooldridge: the following must be taken into account when caching a prepared
    // statement.  Defaults are per JDBC-specification.
    //
    // All of these attributes must match a proposed statement before the statement can be
    // considered "the same" and delivered from the cache.
    private String sql;
